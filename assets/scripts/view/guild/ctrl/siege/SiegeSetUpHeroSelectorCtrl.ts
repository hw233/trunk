 ...
 * @Last Modified time: 2021-05-07 11:04:49
 */
...
@ccclass
@menu("qszc/view/guild/siege/SiegeSetUpHeroSelectorCtrl")
export default class SiegeSetUpHeroSelectorCtrl extends gdk.BasePanel {
    ...
    onEnable() {
        ...
        if (curSiegeCfg.camp_type && curSiegeCfg.camp_type.length > 0) {
            GlobalUtil.setSpriteIcon(this.node, this.groupIcon, `view/guild/texture/icon/${curSiegeCfg.camp_icon}`);
            this.recommendLab.string = `${curSiegeCfg.bonus_des}`
        } else {
            this.groupIcon.parent.active = false
        }
    ...
    initUpHeroData() {
        this.selectHeros = this.model.curUpHeroList(this.curIndex)
        for (let i = 0; i < 6; i++) {
    ...
    attackBtnClick() {
        let stageCfg = ConfigManager.getItemById(Copy_stageCfg, this.siegeModel.curPointCfg.stage_id)
        if (!stageCfg) {
            CC_DEBUG && cc.error("关卡配置不存在")
            return
        }
        JumpUtils.openInstance(stageCfg.id);
        gdk.panel.hide(PanelId.SiegeSetUpHeroSelector)
    }
}
