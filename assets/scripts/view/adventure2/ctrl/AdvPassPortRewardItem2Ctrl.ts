...
import { Adventure_passCfg, Adventure2_passCfg } from '../../../a/config';
...
@ccclass
@menu("qszc/view/adventure2/AdvPassPortRewardItem2Ctrl")
export default class AdvPassPortRewardItem2Ctrl extends UiListItem {
    @property(UiSlotItem)
    ...
    updateRewardState() {
        let recevie1 = NewAdventureUtils.getPassPortRewardState(this.cfg.taskid, 1);
        let recevie2 = NewAdventureUtils.getPassPortRewardState(this.cfg.taskid, 2);
        ...
        passPortItems.forEach((item, idx) => {
            if (item.node.active) {
                ...
                if (!this.adventureModel.isBuyPassPort) {
                    lock.active = true;
                    mask.active = true;
                }
                else {
    ...
    onGetBtnClick() {
        let recevie1 = NewAdventureUtils.getPassPortRewardState(this.cfg.taskid, 1);
        if (recevie1 && !ModelManager.get(NewAdventureModel).isBuyPassPort) {
            //TODO
            // gdk.gui.showMessage('购买通行证后可继续领取');
            let cfgs = ConfigManager.getItemsByField(Adventure2_passCfg, 'cycle', this.cfg.cycle);
            gdk.panel.setArgs(PanelId.AdvPassPortBuyView, cfgs);
            gdk.panel.open(PanelId.AdvPassPortBuyView);
        }
        else {
            let req = new icmsg.Adventure2PassAwardReq();
            req.id = this.cfg.taskid;
            NetManager.send(req, (resp: icmsg.Adventure2PassAwardRsp) => {
