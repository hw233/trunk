 ...
 * @Last Modified time: 2021-08-19 17:43:51
 */
...
@ccclass
@menu("qszc/view/adventure2/NewAdventureSetUpHeroSelectorCtrl")
export default class NewAdventureSetUpHeroSelectorCtrl extends gdk.BasePanel {
    ...
    initOrdealHeroUpData() {
        this.themeheroCfg = ConfigManager.getItemByField(Adventure_themeheroCfg, "type", NewAdventureUtils.actRewardType, { difficulty: this.adventureModel.difficulty })
    ...
    getLocalStr(index: number): string {
        ...
        switch (index) {
            case 0:
                str = 'Adventure_endless_setUpHero_pve'
                break;
            case 1:
    ...
    oneKeyUpHero() {
        ...
        //     }
        // });
        let temDatas = this.model.heroInfos.concat()//.concat(hireList);
    ...
    attackBtnClick() {
        ...
        // let selectPlate = this.adventureModel.endless_selectIndex
        // let curPlate = this.adventureModel.endless_plateIndex
        let stageCfg = ConfigManager.getItemById(Copy_stageCfg, this.adventureModel.endless_stageId)
        if (!stageCfg) {
            CC_DEBUG && cc.error("关卡配置不存在")
        }
        JumpUtils.openInstance(stageCfg.id);
        gdk.panel.hide(PanelId.AdvPlateStagePanel2)
        gdk.panel.hide(PanelId.NewAdventureSetUpHeroSelector)
