...
import AdvPassPortRewardItem2Ctrl from './AdvPassPortRewardItem2Ctrl';
...
import { Adventure2_passCfg } from '../../../a/config';
...
export default class AdvPassPortViewCtrl extends gdk.BasePanel {
    ...
    onEnable() {
        ...
        NetManager.on(icmsg.Adventure2PassAwardRsp.MsgType, this._onPassAwardRsp, this);
    ...
    _initTime() {
        ...
        this.cfgs = ConfigManager.getItemsByField(Adventure2_passCfg, 'cycle', cfg.reward_type);
        if (!this.cfgs || this.cfgs.length <= 0) {
            let c = ConfigManager.getItems(Adventure2_passCfg);
            this.cfgs = ConfigManager.getItemsByField(Adventure2_passCfg, 'cycle', c[c.length - 1].cycle);
        }
    }
        ...
        gdk.Timer.callLater(this, () => {
            ...
            for (let i = 0; i < this.cfgs.length; i++) {
                ...
                if (this.cfgs[i].score[1] <= pass) {
                    if (!NewAdventureUtils.getPassPortRewardState(this.cfgs[i].taskid, 1)) {
                        this.list.scroll_to(Math.max(0, i - 1));
                        this.onTaskScroll();
                        return;
                    }
                    if (this.adModel.isBuyPassPort && !NewAdventureUtils.getPassPortRewardState(this.cfgs[i].taskid, 2)) {
                        this.list.scroll_to(Math.max(0, i - 1));
                        this.onTaskScroll();
                        return;
                    }
    ...
    _updateSpecialItem() {
        let cfgs: Adventure2_passCfg[] = this.cfgs.filter(cfg => {
            return cfg.resident == 1;
        })

        for (let i = 0; i < cfgs.length; i++) {
            if (i != cfgs.length - 1 && NewAdventureUtils.getPassPortRewardState(cfgs[i].taskid, 1) && NewAdventureUtils.getPassPortRewardState(cfgs[0].taskid, 2)) continue;
            else {
                let ctrl = this.specialItem.getChildByName('PassPortRewardItem').getComponent(AdvPassPortRewardItem2Ctrl);
                ctrl.data = cfgs[i];
                ctrl.updateView();
                break;
            }
    ...
    onBuyBtnClick() {
        if (this.adModel.isBuyPassPort) {
            ...
            gdk.panel.setArgs(PanelId.AdvPassPortBuyView2, this.cfgs);
            gdk.panel.open(PanelId.AdvPassPortBuyView2);
        }
    }
    ...
    _onPassAwardRsp(resp: icmsg.Adventure2PassAwardRsp) {
        let cfg = ConfigManager.getItem(Adventure2_passCfg, (cfg: Adventure2_passCfg) => {
