...
import { Adventure2_themeheroCfg, HeroCfg, Store_pushCfg } from '../../../a/config';
...
@ccclass
@menu("qszc/view/adventure2/AdventureGiftView2Ctrl")
export default class AdventureGiftView2Ctrl extends gdk.BasePanel {
    @property(cc.Label)
    ...
    @gdk.binding('adventureModel.GiftTime')
    _updateBtns() {
        ...
        let temCfgs: Adventure2_themeheroCfg[] = ConfigManager.getItemsByField(Adventure2_themeheroCfg, 'type', NewAdventureUtils.actRewardType)

        temCfgs.forEach(cfg => {
            if (cc.js.isNumber(cfg.push) && cfg.push > 0) {
                let len = this.adventureModel.GiftTime.length
                for (let i = 0; i < len; i++) {
                    let data = this.adventureModel.GiftTime[i]
                    if (data.giftId == cfg.push) {
                        let temcfg = ConfigManager.getItemById(Store_pushCfg, cfg.push);
                        let startTime = data.startTime;
                        if (cfg && startTime > 0 && startTime + temcfg.duration > GlobalUtil.getServerTime() / 1000) {
                            cfgs.push(temcfg);
                            datas.push(data);
                        }
                        break;
