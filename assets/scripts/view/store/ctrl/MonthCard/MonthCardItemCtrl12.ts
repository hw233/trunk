...
@ccclass
@menu("qszc/view/store/MonthCard/MonthCardItemCtrl12")
export default class MonthCardItemCtrl12 extends cc.Component {
    ...
    updateView() {
        this.data = this.storeModel.getMonthCardInfo(500001);
    ...
    _updateDatas(isReset: boolean = false) {
        ...
        for (let i = 0; i < total; i++) {
            ...
            if (this._leftDay > 0 && this.data) {
                if (i == this.cfg.day - (this._leftDay % this.cfg.day == 0 ? this.cfg.day : this._leftDay % this.cfg.day)) {
                    index = i
                }
            }
            rewards.push(info)
