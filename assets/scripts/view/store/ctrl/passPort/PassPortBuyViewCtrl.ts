...
import { GlobalCfg, PassCfg, Store_miscCfg } from '../../../../a/config';
/** 
  * @Author: jiangping  
  * @Description: 
  * @Date: 2020-07-01 20:06:45 
  */// Learn TypeScript:
...
@ccclass
@menu("qszc/view/store/passPort/PassPortBuyViewCtrl")
export default class PassPortBuyViewCtrl extends gdk.BasePanel {
    ...
    onEnable() {
        ...
        let cfg = ConfigManager.getItemById(Store_miscCfg, 700001);
        this.buyBtn.node.getChildByName('label').getComponent(cc.Label).string = `￥${cfg.RMB_cost}`;
        let addScore = ConfigManager.getItemByField(GlobalCfg, 'key', 'pass_buy_score').value[0];
        this.cfgs.forEach((cfg, idx) => {
            if (cfg.score <= ModelManager.get(RoleModel).pass + addScore) this.lockIdx = idx + 1;
        });
        this.unlockTips.string = StringUtils.format(gdk.i18n.t('i18n:ADVENTURE_TIP48'), addScore, this.lockIdx);
    ...
    onBuyBtnClick() {
        ...
        GlobalUtil.openAskPanel({
            ...
            descText: StringUtils.format(gdk.i18n.t('i18n:MINECOPY_PASSPORT_TIP2'), cfg.RMB_cost),
            sureText: gdk.i18n.t('i18n:MINECOPY_PASSPORT_TIP3'),
            sureCb: () => {
                let req = new icmsg.PayOrderReq();
                req.paymentId = 700001;
                NetManager.send(req);
            }
        });
        this.close();
