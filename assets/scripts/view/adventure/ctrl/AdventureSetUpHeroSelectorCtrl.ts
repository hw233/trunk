import {
    AdventureCfg, Adventure_hireCfg,
 ...
 * @Last Modified time: 2021-05-07 13:49:31
 */
...
@ccclass
@menu("qszc/view/adventure/AdventureSetUpHeroSelectorCtrl")
export default class AdventureSetUpHeroSelectorCtrl extends gdk.BasePanel {
    ...
    heroUpLb1: cc.RichText = null;

    @property(cc.RichText)
    heroUpLb2: cc.RichText = null;
    ...
    themeheroCfg: Adventure_themeheroCfg;
    cfgHeroId: number[] = [];
    onEnable() {
        this.upHeroIndex = -1;
        ...
        let hireList = []
        //添加雇佣兵数据
        this.adventureModel.giveHeros.forEach(hero => {
            // 创建BagItem数据
            let d = AdventureUtils.createHeroBagItem(hero)
        ...
        this.datas = this.model.heroInfos.concat(hireList);
    ...
    //设置关卡推荐英雄信息
    initOrdealHeroUpData() {
        this.themeheroCfg = ConfigManager.getItemByField(Adventure_themeheroCfg, "type", AdventureUtils.actRewardType, { difficulty: this.adventureModel.difficulty })
        let strs = this.themeheroCfg.theme_hero.split('|');
        ...
        this.heroUpLb1.string = strs.length > 0 ? strs[0].split(';')[1] : '';
        this.heroUpLb2.string = strs.length > 1 ? strs[1].split(';')[1] : '';
        this.heroUpLb1.node.color = cc.color('D0C1C1');
        this.heroUpLb2.node.color = cc.color('D0C1C1');
    }

    //检测关卡推荐上阵状态
    refreshOrdealUpHeroData() {
        //检测英雄是否上阵
        let strs = this.themeheroCfg.theme_hero.split('|');
        for (let i = 0; i < strs.length; i++) {
            let heroId = Number(strs[i].split(';')[0]);
            let node = this.heroUpNode.children[i]
            let state: 0 | 1 = 0;
            if (this.selectHeroItemIds.indexOf(heroId) < 0) {
                state = 1;
            }
            GlobalUtil.setAllNodeGray(node, state);
            if (i == 0) {
                this.heroUpLb1.node.color = state == 1 ? cc.color('D0C1C1') : cc.color('FFFFFF')
                state == 0 ? this.heroUpLb1.string = strs[i].split(';')[1] : 0;
                //GlobalUtil.setAllNodeGray(this.heroUpLb1.node, state);
            } else {
                this.heroUpLb2.node.color = state == 1 ? cc.color('D0C1C1') : cc.color('FFFFFF')
                state == 0 ? this.heroUpLb2.string = strs[i].split(';')[1] : 0;
                //GlobalUtil.setAllNodeGray(this.heroUpLb2.node, state);
            }
        }
    }
    ...
    sendHeroListChangeInfo() {
        this.adventureModel.pveHireHeros = {}
        for (let i = 0; i < this.selectHeros.length; i++) {
            if (this.selectHeros[i] > 900000) {
                this.adventureModel.pveHireHeros[this.selectHeros[i]] = AdventureUtils.getHireHero(this.selectHeros[i])
            }
        }
    ...
    refreshGroupBuffInfo() {
        ...
        if (!JumpUtils.ifSysOpen(2853)) {
            ...
            if (systemCfg) {
                ...
                this.lockNode.string = StringUtils.format(gdk.i18n.t("i18n:ADVENTURE_TIP22"), stageCfg.name.split(' ')[0])//'通关主线' + stageCfg.name.split(' ')[0] + '解锁';
            }
            ...
            if (i < this.copyCfg.num) {
                ...
                if (tem) {
                    ...
                    if (stageCfg) {
                        ...
                        num.string = StringUtils.format(gdk.i18n.t("i18n:ADVENTURE_TIP23"), tem[0])//'通关' + tem[0] + '开放';
                    }
                }
                lock.active = true;
    ...
    downHero(e: Event, index) {
        ...
        if (roleHeroItem.active) {
            if (num <= 1) {
                gdk.gui.showMessage(`${gdk.i18n.t("i18n:ADVENTURE_TIP24")}`)
                return
            }
            let ctrl = roleHeroItem.getComponent(RoleHeroItemCtrl2);
    ...
    pageSelect(event, index, refresh: boolean = false) {
        ...
        this.scrollView.node.parent.setPosition(cc.v2(0, 45))
        this.scrollView.node.parent.height = 420;
    ...
    getLocalStr(index: number): string {
        ...
        switch (index) {
            case 0:
                str = 'Adventure_setUpHero_pve'
                break;
            case 1:
                str = 'Adventure_setUpHero_pvp_arena'
                break;
            case 2:
                str = 'Adventure_setUpHero_pve_copy'
                break;
        }
        return str;
    }
    ...
    openHaloView() {
        if (!JumpUtils.ifSysOpen(2853)) {
            ...
            if (systemCfg) {
                let stageCfg = ConfigManager.getItemById(Copy_stageCfg, systemCfg.fbId)
                let str = StringUtils.format(gdk.i18n.t("i18n:ADVENTURE_TIP22"), stageCfg.name.split(' ')[0])//'通关主线' + stageCfg.name.split(' ')[0] + '解锁';
    ...
    oneKeyUpHero() {
        ...
        let temDatas = this.model.heroInfos.concat(hireList);
    ...
    attackBtnClick() {
        ...
        let advCfg = ConfigManager.getItemByField(AdventureCfg, "difficulty", this.adventureModel.difficulty, { type: AdventureUtils.actRewardType, layer_id: this.adventureModel.layerId, plate: selectPlate })
        let stageCfg = ConfigManager.getItemById(Copy_stageCfg, advCfg.event_id)
        if (selectPlate != curPlate) {
            if (!stageCfg) {
                CC_DEBUG && cc.error("关卡配置不存在")
            }
            JumpUtils.openInstance(stageCfg.id);
            gdk.panel.hide(PanelId.AdvPlateStagePanel)
            gdk.panel.hide(PanelId.AdventureSetUpHeroSelector)
            // let msg = new AdventurePlateEnterReq()
