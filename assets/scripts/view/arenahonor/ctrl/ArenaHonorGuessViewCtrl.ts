...
@ccclass
@menu("qszc/view/arenaHonor/ArenaHonorGuessViewCtrl")
export default class ArenaHonorGuessViewCtrl extends gdk.BasePanel {
    ...
    get cModel(): ArenaHonorModel { return ModelManager.get(ArenaHonorModel); }
    ...
    onEnable() {
        ...
        this.curProId = ArenaHonorUtils.getCurProgressId();
        this.guessList = [];
        let cfg = ConfigManager.getItemById(Arenahonor_progressCfg, this.curProId);
    ...
    onBetBtnClick(e, data) {
        ...
        gdk.panel.setArgs(PanelId.ArenaHonorBetView, [index, info]);
        gdk.panel.open(PanelId.ArenaHonorBetView);
    }
    ...
    onFightBtnClick() {
        ...
        JumpUtils.openPveArenaScene([player.id, 0, player], player.name, 'ARENAHONOR_GUESS');
    }
    ...
    onTabMenuSelect(e, data) {
        ...
        this.players.forEach((p, idx) => {
            ...
            guildName.string = this.cModel.guildNameMap[info.guildId];
            ...
            if (!infos.guess) {
                ...
                GlobalUtil.setSpriteIcon(this.node, cc.find('icon', betState), GlobalUtil.getIconById(31));
    ...
    _updateMoney() {
        GlobalUtil.setSpriteIcon(this.node, this.moneyNode.getChildByName('icon'), GlobalUtil.getIconById(31));
        this.moneyNode.getChildByName('num').getComponent(cc.Label).string = BagUtils.getItemNumById(31) + '';
    }
    ...
    openGuessReprotViewBtnClick() {
        gdk.panel.open(PanelId.ArenahonorGuessReportView);
    }
