...
import { Activity_newtopupCfg, ActivityCfg } from '../../../../a/config';
...
@ccclass
@menu("qszc/view/act/newLcdl/NewLcdlViewCtrl")
export default class NewLcdlViewCtrl extends gdk.BasePanel {
    @property(cc.Label)
    ...
    _updateView() {
        ...
        gdk.Timer.callLater(this, () => {
            ...
            let monthlyRecharge = ModelManager.get(ActivityModel).newTopUpRecharge;
            for (let i = 0; i < cfgs.length; i++) {
                if (monthlyRecharge >= cfgs[i].money) {
                    if (!ActivityUtils.getNewLcdlRewardState(cfgs[i].index)) {
