 ...
 * @Last Modified time: 2021-08-26 10:47:04
 */
...
@ccclass
@menu("qszc/view/peak/PeakHeroDetailSkillCtrl")
export default class PeakHeroDetailSkillCtrl extends gdk.BasePanel {
    ...
    updateView(cfg: HeroCfg, curType: number, hbInfo?: HBItemType) {
    ...
    onSwitchCareer() {
        ...
        if (this._careerLines.length > 1) {
            ...
            let msg = new icmsg.PeakHeroCareerReq()
            msg.careerId = temCfg.career_id;
            msg.heroId = this._heroCfg.id;
            NetManager.send(msg, (rsp: icmsg.PeakHeroCareerRsp) => {
                //刷新数据;
                this.peakModel.peakStateInfo.heroes.forEach(hero => {
                    if (hero.typeId == this._heroCfg.id) {
                        hero.careerType = temCfg.career_type;
                    }
                })
