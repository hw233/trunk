...
import PeakHeroDetailSkillCtrl from './PeakHeroDetailSkillCtrl';
 ...
 * @Last Modified time: 2021-03-03 17:37:54
 */
...
@ccclass
@menu("qszc/view/peak/PeakHeroDetailViewCtrl")
export default class PeakHeroDetailViewCtrl extends gdk.BasePanel {
    ...
    onEnable() {
        ...
        let args = gdk.panel.getArgs(PanelId.PeakHeroDetailView)
        if (args) {
            this.curType = args[0];
        }
        // 属性文本
    ...
    checkArgs() {
        ...
        if (panelId) {
            gdk.panel.open(
                panelId,
                (node: cc.Node) => {
                    if (panelId.__id__ == PanelId.PeakHeroDetailSkill.__id__) {
                        let ctrl = node.getComponent(PeakHeroDetailSkillCtrl)
                        ctrl.updateView(this._heroCfg, this.curType, this.hbInfo)
                    }
                },
                this,
                {
    ...
    initHeroInfo(heroId: number, careerType: number, careerId?: number) {
    ...
    _updateUpInfo() {
        ...
        let color = ConfigManager.getItemById(Hero_starCfg, this.peakModel.starNum).color;
    ...
    _updateStar(cfg: HeroCfg) {
        let starTxt = "";
        starTxt = this.peakModel.starNum > 5 ? '2'.repeat(this.peakModel.starNum - 5) : '1'.repeat(this.peakModel.starNum);
    ...
    _getHeroAttr(careerId) {
        ...
        // }
        let msg = new icmsg.PeakHeroImageReq()
        msg.careerType = this.careerType;
        msg.heroId = this._heroCfg.id;
        msg.careerId = careerId;
        NetManager.send(msg, (rsp: icmsg.PeakHeroImageRsp) => {
            //刷新数据
            // 0 生命 1 攻速 2 攻击 3防御
