import {
    Copy_towerhaloCfg, Eternal_globalCfg, GroupCfg,
    HeroCfg
} from '../../../../a/config';
 ...
 * @Last Modified time: 2021-05-07 11:05:54
 */
...
@ccclass
@menu("qszc/view/act/eternalCopy/EternalHeroHaloViewCtrl")
export default class EternalHeroHaloViewCtrl extends gdk.BasePanel {
    ...
    onEnable() {
        ...
        let cfg = ConfigManager.getItemByField(Eternal_globalCfg, 'key', 'camp');
        this.tipStr.string = cfg.value[0] + '%';
    ...
    initGroupData() {
        ...
        let arg = gdk.panel.getArgs(PanelId.EternalHeroHaloView);
        if (arg) {
    ...
    setGroupIconState() {
        ...
        for (let i = 0; i < this.iconNodes.length; i++) {
            let node = this.iconNodes[i];
            let label = node.getChildByName('lv').getComponent(cc.Label);
            let icon = node.getChildByName('icon');
            let state: 0 | 1 = 1;
            if (this.a[i + 1]) {
                label.string = this.a[i + 1] + '';
                if (i + 1 == this.maxType || i + 1 <= 2) {
                    state = 0
                }
            } else {
                label.string = '0'
    ...
    setShuxingData() {
        ...
        for (let i = 0; i < this.attributesNodes.length; i++) {
            ...
            if (temCfgs[i]) {
                node.active = true;
                let temStrs = temCfgs[i].des1.split(';');
                ...
                let num4 = node.getChildByName('num4').getComponent(cc.Label);
    ...
    setGroupData() {
        ...
        for (let i = 0; i < this.groupNodes.length; i++) {
            ...
            tips.active = false//have;
            num1.node.active = true//!!energyAdvanceCfg && energyAdvanceCfg.camp;
            if (num1.node.active) {
