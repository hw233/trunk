...
@ccclass
@menu("qszc/view/act/cave/CaveMainCtrl")
export default class CaveMainCtrl extends gdk.BasePanel {
    @property(cc.Node)
    tabBtns: cc.Node[] = [];
    ...
    onEnable() {
        ...
        gdk.e.on("popup#CaveGiftView#close", this._hideLinkGameGiftViewFunc, this);
    }
    onDisable() {
        gdk.e.off("popup#CaveGiftView#close", this._hideLinkGameGiftViewFunc, this);
    ...
    _updateBtns() {
        if (ActivityUtils.getCaveGiftDatas().length == 0) {
    ...
    _hideLinkGameGiftViewFunc() {
        this.tabBtns[1].active = ActivityUtils.getCaveGiftDatas().length > 0;
