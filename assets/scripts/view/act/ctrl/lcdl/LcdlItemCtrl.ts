...
import { Activity_top_upCfg, General_weaponCfg } from '../../../../a/config';
/** 
  * @Author: jiangping  
  * @Description: 
...
@ccclass
@menu("qszc/view/act/lcdl/LcdlItemCtrl")
export default class LcdlItemCtrl extends UiListItem {
  @property(cc.Label)
  ...
  _updateState() {
    ...
    if (model.monthlyRecharge >= this.cfg.money) {
      if (ActivityUtils.getLcdlRewardState(this.cfg.index)) {
        ...
        this.progressLabel.string = `(${SdkTool.tool.getRealRMBCost(model.monthlyRecharge)}/${SdkTool.tool.getRealRMBCost(this.cfg.money)})`;
      }
    }
    else {
      this.mask.active = false;
  ...
  onGetBtnClick() {
    if (!ActUtil.ifActOpen(11)) {
      ...
      let req = new icmsg.ActivityTopUpAwardReq();
      req.paySum = this.cfg.money;
      NetManager.send(req, (resp: icmsg.ActivityTopUpAwardRsp) => {
