...
/**sysId-panelId */
const SailingActivityPanelInfo: any = {
    2944: { panelId: PanelId.SailingView, closeBtn: true, redPointCb: RedPointUtils.has_sailing_pay_reward, args: null },  //黄金寻宝船
    2945: { panelId: PanelId.SailingTreasurePanel, closeBtn: true, redPointCb: RedPointUtils.has_sailing_treasure_tab_redPoint, args: null },  //远航寻宝
}

@ccclass
//@menu("qszc/view/act/ActivityMainViewCtrl")
export default class SailingActivityMainViewCtrl extends gdk.BasePanel {
    @property(cc.Node)
    ...
    onDisable() {
        // 关闭打开或打开中的子界面
        for (let key in SailingActivityPanelInfo) {
            if (SailingActivityPanelInfo[key]) {
                gdk.panel.hide(SailingActivityPanelInfo[key].panelId);
            }
        }
        this.curCfg = null;
        this.curIdx = null;
        ...
        for (let i = 0; i < this.pageIndex.length; i++) {
            if (idxs.indexOf(this.pageIndex[i]) == -1) {
                ...
                if (toggle) {
                    ...
                    if (!cfg) continue;
                    let redPointCb = SailingActivityPanelInfo[cfg.systemid]['redPointCb'];
                    let args = SailingActivityPanelInfo[cfg.systemid]['args'];
    ...
    _initCfgs() {
        this.actCfgs = ConfigManager.getItems(MainInterface_sortCfg, (cfg: MainInterface_sortCfg) => {
            if (SailingActivityPanelInfo[cfg.systemid] && JumpUtils.ifSysOpen(cfg.systemid)) return true;
        });
    ...
    _preloadPanel() {
        this.actCfgs.forEach(cfg => {
            if (JumpUtils.ifSysOpen(cfg.systemid)) {
                let panelId: gdk.PanelValue = SailingActivityPanelInfo[cfg.systemid]['panelId'];
    ...
    _initPageItem() {
        ...
        this.actCfgs.forEach((cfg, idx) => {
            ...
            ctrl.updateView(cfg, SailingActivityPanelInfo[cfg.systemid]['redPointCb'], SailingActivityPanelInfo[cfg.systemid]['args']);
    ...
    _updatePanel(cfg: MainInterface_sortCfg) {
        if (this.curCfg) {
            let info = SailingActivityPanelInfo[this.curCfg.systemid];
