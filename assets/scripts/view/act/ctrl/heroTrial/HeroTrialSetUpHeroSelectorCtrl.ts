 ...
 * @Last Modified time: 2021-08-19 17:42:48
 */
...
@ccclass
@menu("qszc/view/role2/selector/HeroTrialSetUpHeroSelectorCtrl")
export default class HeroTrialSetUpHeroSelectorCtrl extends gdk.BasePanel {
    ...
    ordealCfg: OrdealCfg | Newordeal_ordealCfg;
    cfgHeroId: number[] = [];
    onEnable() {
        this.upHeroIndex = -1;
        ...
        let arg = gdk.panel.getArgs(PanelId.HeroTrialSetUpHeroSelector);
        if (arg) {
    ...
    initOrdealHeroUpData() {
        let strs = this.ordealCfg.theme_hero.split('|');
    ...
    attackBtnClick() {
        this.close()
        if (this.ordealCfg.copy_id == CopyType.HeroTrial) {
            gdk.panel.hide(PanelId.ActivityMainView);
            gdk.panel.hide(PanelId.HeroTrialStageRewardView);
        } else if (this.ordealCfg.copy_id == CopyType.NewHeroTrial) {
            gdk.panel.hide(PanelId.ActivityMainView);
            gdk.panel.hide(PanelId.NewHeroTrialStageRewardView);
            //gdk.panel.hide(PanelId.NewHeroTrialMonsterView);
        }
        JumpUtils.openInstance(this.ordealCfg.round)
