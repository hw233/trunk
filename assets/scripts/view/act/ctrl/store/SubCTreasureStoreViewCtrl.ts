 ...
 * @Last Modified time: 2021-04-29 11:11:28
 */

const { ccclass, property, menu } = cc._decorator;

@ccclass
@menu("qszc/view/act/store/SubCTreasureStoreViewCtrl")
export default class SubCTreasureStoreViewCtrl extends gdk.BasePanel {
    @property(cc.Label)
    ...
    _updateView() {
        ...
        let cfgs = ConfigManager.getItems(Operation_storeCfg, (cfg: Operation_storeCfg) => {
            if (cfg.reward_type == this.cfg.reward_type && cfg.page == 4) return true;
        });
        if (!cfgs || cfgs.length <= 0) {
            let c = ConfigManager.getItemsByField(Operation_storeCfg, 'page', 4);
            cfgs = ConfigManager.getItems(Operation_storeCfg, (cfg: Operation_storeCfg) => {
                if (cfg.reward_type == c[c.length - 1].reward_type && cfg.page == 4) {
                    return true;
                }
            })
        }
