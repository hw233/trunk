 ...
 * @Last Modified time: 2020-12-22 11:36:39
 */

const { ccclass, property, menu } = cc._decorator;

@ccclass
@menu("qszc/view/act/store/SubAdvStoreViewCtrl")
export default class SubAdvStoreViewCtrl extends gdk.BasePanel {
    @property(cc.Label)
    ...
    _updateView() {
        ...
        let cfgs = ConfigManager.getItems(Operation_storeCfg, (cfg: Operation_storeCfg) => {
            if (cfg.reward_type == this.cfg.reward_type && cfg.page == 3) return true;
        });
        if (!cfgs || cfgs.length <= 0) {
            let c = ConfigManager.getItemsByField(Operation_storeCfg, 'page', 3);
            cfgs = ConfigManager.getItemsByField(Operation_storeCfg, 'reward_type', c[c.length - 1].reward_type);
        }
        let datas = []
        let outDatas = []
