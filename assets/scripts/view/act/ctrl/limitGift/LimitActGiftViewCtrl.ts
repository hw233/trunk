...
@ccclass
@menu("qszc/view/store/LimitActGiftViewCtrl")
export default class LimitActGiftViewCtrl extends gdk.BasePanel {
    ...
    iconStrs: string[] = ['xssj_zangshidaoju02', 'xssj_zangshidaoju03', 'xssj_zangshidaoju01'];
    titleStrs: string[] = ['xssj_mingchen02', 'xssj_mingchen03', 'xssj_mingchen01']
    // update (dt) {}
    onEnable() {

        gdk.e.on(StoreEventId.UPDATE_PAY_SUCC, this._updatePaySucc, this)

        let args = gdk.panel.getArgs(PanelId.LimitActGiftView)
        if (args) {
            this.type = args[0]
        }
        //let id = args[0]
    ...
    initGiftData() {
        ...
        this.aModel.LimitGiftDatas.forEach(data => {
            if ((curTime < data.endTime && data.state == 0) || data.state == 1) {
                ...
                if (temType.indexOf(cfg.gift_type) < 0) {
                    this.allLimitGift.push(data);
                    temType.push(cfg.gift_type)
                }
            }
        })
    ...
    onClickRewardBtn() {
        if (this.rewardBtnState) return;
        this.rewardBtnState = true;
