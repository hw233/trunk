...
@ccclass
@menu("qszc/view/act/luckyTwist/LuckTwistEggBallCtrl")
export default class LuckTwistEggBallCtrl extends cc.Component {
    @property(cc.Node)
    icon: cc.Node = null;

    isNeedPush: boolean = false;
    ...
    playAni() {
        ...
        gdk.Timer.once(5000, this, () => {
            ...
            gdk.Timer.once(3000, this, () => {
                ...
                rigid.linearVelocity = cc.v2();
                if (this.isJackPot) {
                    rigid.onPreSolve = (contact, selfCollider, otherCollider) => {
                        if (otherCollider.tag == 99) {
                            contact.disabled = true;
                        }
                    };
                    rigid.onPostSolve = (contact, selfCollider, otherCollider) => {
                        if (otherCollider.tag == 88) {
                            rigid.onPreSolve = null;
                            rigid.onPostSolve = null;
                            rigid.angularVelocity = 0;
