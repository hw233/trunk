...
/**sysId-panelId */
const GuardianCallActivityPanelInfo: any = {
    2908: { panelId: PanelId.GuardianCallRewardView, closeBtn: false, redPointCb: RedPointUtils.has_guardianCallReward_red, args: null },  //召唤有礼
}

@ccclass
//@menu("qszc/view/act/ActivityMainViewCtrl")
export default class CallRewardActivityMainViewCtrl extends gdk.BasePanel {
    @property(cc.Node)
    ...
    onDisable() {
        // 关闭打开或打开中的子界面
        for (let key in GuardianCallActivityPanelInfo) {
            if (GuardianCallActivityPanelInfo[key]) {
                gdk.panel.hide(GuardianCallActivityPanelInfo[key].panelId);
            }
        }
        this.curCfg = null;
        this.curIdx = null;
        ...
        for (let i = 0; i < this.pageIndex.length; i++) {
            if (idxs.indexOf(this.pageIndex[i]) == -1) {
                ...
                if (toggle) {
                    ...
                    if (!cfg) continue;
                    let redPointCb = GuardianCallActivityPanelInfo[cfg.systemid]['redPointCb'];
                    let args = GuardianCallActivityPanelInfo[cfg.systemid]['args'];
    ...
    _initCfgs() {
        this.actCfgs = ConfigManager.getItems(MainInterface_sortCfg, (cfg: MainInterface_sortCfg) => {
            if (GuardianCallActivityPanelInfo[cfg.systemid] && JumpUtils.ifSysOpen(cfg.systemid)) return true;
        });
    ...
    _preloadPanel() {
        this.actCfgs.forEach(cfg => {
            if (JumpUtils.ifSysOpen(cfg.systemid)) {
                let panelId: gdk.PanelValue = GuardianCallActivityPanelInfo[cfg.systemid]['panelId'];
    ...
    _initPageItem() {
        ...
        this.actCfgs.forEach((cfg, idx) => {
            ...
            ctrl.updateView(cfg, GuardianCallActivityPanelInfo[cfg.systemid]['redPointCb'], GuardianCallActivityPanelInfo[cfg.systemid]['args']);
    ...
    _updatePanel(cfg: MainInterface_sortCfg) {
        if (this.curCfg) {
            let info = GuardianCallActivityPanelInfo[this.curCfg.systemid];
