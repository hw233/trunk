...
/** 
 * @Description:跨服狂欢排行榜Item
 * @Author: yaozu.hu
 * @Date: 2020-10-21 14:50:32
 * @Last Modified by: yaozu.hu
 * @Last Modified time: 2021-05-31 15:17:39
 */
const { ccclass, property, menu } = cc._decorator;

@ccclass
export default class CServerPersonRankItemCtrl extends UiListItem {

    @property(cc.Label)
    nameLabel: cc.Label = null;
    @property(cc.Label)
    serverLabel: cc.Label = null;
    ...
    async updateItemView(data) {
        let d: icmsg.RoleBrief = data.brief;
        let value = data.value;
        let rank = data.rank;
        this.nameLabel.string = d.name;
        this.lvLabel.string = '.' + d.level;
        await ModelManager.get(ServerModel).reqServerNameByIds([data.brief.id]);
        this.serverLabel.string = `[S${GlobalUtil.getSeverIdByPlayerId(data.brief.id)}] ${ModelManager.get(ServerModel).serverNameMap[Math.floor(data.brief.id / 100000)]}`;
