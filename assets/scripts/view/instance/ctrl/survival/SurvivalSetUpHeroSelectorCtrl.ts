 ...
 * @Last Modified time: 2021-08-31 18:11:30
 */
...
@ccclass
@menu("qszc/view/instance/SurvivalSetUpHeroSelectorCtrl")
export default class SurvivalSetUpHeroSelectorCtrl extends gdk.BasePanel {
    ...
    effectPos1: cc.Vec2[] = [cc.v2(-264, -130), cc.v2(-157, -130), cc.v2(-52, -130), cc.v2(52, -130), cc.v2(157, -130), cc.v2(264, -130), cc.v2(-264, -236), cc.v2(-157, -236), cc.v2(-52, -236)]
    ...
    onEnable() {
        ...
        //     return false
        // })
        let tems = ConfigManager.getItemsByField(Copy_towerlistCfg, 'num', 9);
        let view = gdk.gui.getCurrentView();
        let model = view.getComponent(PveSceneCtrl).model;
        this.maxUpHeroNum_Woker = model.eliteStageUtil.maxUpHeroNum_Woker
        this.maxUpHeroNum = model.eliteStageUtil.maxUpHeroNum
        this.copyCfg = tems[tems.length - 1];
        this.datas = this.model.heroInfos.concat();
        let temMercenaryList = []

        //添加雇佣兵数据
        ModelManager.get(MercenaryModel).borrowedListHero.forEach(hero => {
            // 创建BagItem数据
            let d = HeroUtils.createMercenaryHeroBagItem(hero);
            temMercenaryList.push(d);
        });

        this.datas = this.datas.concat(temMercenaryList);
        this.datas.sort(this.sortFunc1)
        gdk.e.on(MercenaryEventId.MERCENARY_GET_REFRESH_HERO, this._refreshHeroData, this)
    ...
    _refreshHeroData() {
        ...
        // for (let i = this.selectHeros.length - 1; i >= 0; i--) {
        //     let have = false;
        //     let heroId = this.selectHeros[i];
        //     temMercenaryList.forEach(mercenary => {
        //         if (mercenary.series == heroId) {
        //             have = true;
        //         }
        //     })
        //     if (!have) {
        ...
        //         this.selectHeroInfos.splice(i, 1)
        //         this.selectHeroInfos.push(null);
        //         this.selectHeroItemIds.splice(i, 1)
        ...
        let addMercenaryList = []
        for (let i = this.selectHeros.length - 1; i >= 0; i--) {
            let heroId = this.selectHeros[i];
            if (heroId <= 0) {
                for (let j = 0; j < temMercenaryList.length; j++) {
                    let tem = temMercenaryList[j]
                    if (addMercenaryList.indexOf(tem.series) < 0 && this.selectHeroItemIds.indexOf(tem.itemId) < 0) {
                        this.selectHeros[i] = tem.series;
                        this.selectHeroInfos[i] = tem;
                        this.selectHeroItemIds[i] = tem.itemId
                        break;
                    }
                }
            }
    ...
    initUpHeroData() {
        ...
        this.copyModel.survivalStateMsg.heroes.forEach(data => {
            let heroId = data.typeId == 1 ? data.heroId + 600000 : data.heroId
            this.selectHeros.push(heroId);
        })

        for (let i = 0; i < this.maxUpHeroNum; i++) {
            this.selectHeroInfos[i] = null;
            this.selectHeroItemIds[i] = -1;
        }
        let sendInfo = this.copyModel.survivalNeedSend;//false;
        if (!this.selectHeros || this.selectHeros.length == 0) {
            ...
            if (this.selectHeros.length < this.maxUpHeroNum) {
                let num = this.maxUpHeroNum - this.selectHeros.length;
                for (let i = 0; i < num; i++) {
                    this.selectHeros.push(0);
                }
            }
        ...
        this.copyModel.survivalNeedSend = false;

        this.datas.forEach(info => {
            let index = this.selectHeros.indexOf(info.series)
            if (index >= 0 && this.selectHeroItemIds.indexOf(info.itemId) < 0) {
        ...
        for (let i = this.selectHeros.length - 1; i >= 0; i--) {
            let heroId = this.selectHeros[i]
            let info = this.selectHeroInfos[i];
            if (info == null && heroId > 0) {
                change = true;
                this.selectHeros.splice(i, 1)
                this.selectHeros.push(0);
                this.selectHeroInfos.splice(i, 1)
    ...
    sendHeroListChangeInfo() {
        let msg = new icmsg.SurvivalHeroOnReq();
        //msg.type = this.curIndex + 1;
        let temData: icmsg.SurvivalHeroesInfo[] = [];
        this.selectHeros.forEach(id => {
            let tem = new icmsg.SurvivalHeroesInfo()
            tem.typeId = id > 600000 ? 1 : 0;
            tem.heroId = id > 600000 ? id - 600000 : id;
            temData.push(tem);
        })
        msg.heros = temData;

        NetManager.send(msg, (rsp: icmsg.SurvivalHeroOnRsp) => {
            if (!cc.isValid(this.node)) return;
            this.copyModel.survivalStateMsg.heroes = rsp.heros;
            ...
            this.copyModel.SurvivalCopyUpHeroList['' + ctrl.model.stageConfig.subtype] = null;
    ...
    refreshGroupBuffInfo() {
        ...
        if (!JumpUtils.ifSysOpen(2853)) {
            ...
            if (systemCfg) {
                ...
                this.lockNode.string = '通关主线' + stageCfg.name.split(' ')[0] + '解锁';
            }
    ...
    downHero(e: Event, index) {
        ...
        if (roleHeroItem.active) {
            if (num <= 1) {
                gdk.gui.showMessage('至少需要上阵一个英雄')
                return
            }
            let ctrl = roleHeroItem.getComponent(RoleHeroItemCtrl2);
    ...
    _selectItem(data: any, index) {
        ...
        if (i >= this.maxUpHeroNum) {//(i >= this.copyCfg.num)
            gdk.gui.showMessage('当前上阵英雄已满，请先下阵英雄')
            return;
        }

        // 雇佣英雄数量限制
        if (info.series > 600000 && this.maxUpHeroNum_Woker > 0) {
            // 雇佣兵限制数量大于0
            let count = 0;
            this.selectHeros.forEach(heroId => {
                if (heroId > 600000) {
            ...
            if (count >= this.maxUpHeroNum_Woker) {
                // 超过限制的数量
                let desc = gdk.i18n.t("i18n:PVE_SURVIVAL_LIMIT_WORKER_NUM_TIP");
                let text = StringUtils.replace(desc, "@number", `${3}`);
    ...
    getLocalStr(index: number): string {
        ...
        // }
        // return str;
        return index == 0 ? 'Role_setUpHero_pve' : 'Role_setUpHero_def';
    }
    ...
    oneKeyUpHero() {
        ...
        for (let i = 0; i < this.selectHeros.length; i++) {
            let heroId = this.selectHeros[i];
            if (i < this.maxUpHeroNum && heroId <= 0) {
                let index = 0;
                // let queshao = -1;
                // let queshaoType = '';
                let add = false;
