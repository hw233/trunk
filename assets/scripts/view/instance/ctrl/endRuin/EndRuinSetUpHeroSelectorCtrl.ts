 ...
 * @Last Modified time: 2021-08-19 17:44:42
 */
...
@ccclass
@menu("qszc/view/role2/selector/EndRuinSetUpHeroSelectorCtrl")
export default class EndRuinSetUpHeroSelectorCtrl extends gdk.BasePanel {
    ...
    onEnable() {
        ...
        let arg = gdk.panel.getArgs(PanelId.EndRuinSetUpHeroSelector);
        if (arg) {
            this.chapterNum = arg[0];
            this.stageCfg = arg[1];
            // this.initOrdealHeroUpData();
            // this.pageSelect(null, 0);
        }
        this.conditionNode.active = this.stageCfg != null;
        if (this.stageCfg) {
    ...
    attackBtnClick() {
        ...
        if (this.stageCfg) {
            ...
            //获取战略玩家数据
            //JumpUtils.openArena(this.stageCfg.id);
            let playerInfo = this.copyModel.endRuinPvpChapterInfo.player
            let player = new icmsg.ArenaPlayer();
            player.id = playerInfo.id;
            player.robotId = null;
            player.name = playerInfo.name;
            player.head = playerInfo.head;
            player.frame = playerInfo.headFrame;
            player.level = playerInfo.level;
            player.power = playerInfo.power;
            JumpUtils.openPveArenaScene([player.id, 0, player], player.name, 'ENDRUIN');
        }
