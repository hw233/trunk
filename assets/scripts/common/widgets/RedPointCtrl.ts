 ...
 * @Last Modified time: 2020-12-18 10:23:43
 */
const { ccclass, property, menu } = cc._decorator;
const POOL_NAME: string = 'red_point_node';

@ccclass
@menu("qszc/common/widgets/RedPointCtrl")
export default class RedPointCtrl extends cc.Component {
    ...
    _updateViewLater() {
                ...
                if (!cc.isValid(target)) {
                    ...
                    if (!cc.isValid(comp)) {
                        ...
                        if (typeof (handler) !== 'function') {
                            ...
                            // 调用函数并获取返回值
                            b = b && handler.call(comp, h.customEventData, this.node);
                        }
                    }
                }
            }
        ...
        if (b && RedPointUtils.eval_expr(this.andIds, this.orIds)) {
            ...
            //设置默认红点图片
            if (this.redNode.getComponent(cc.Sprite).spriteFrame == null) {
                GlobalUtil.setSpriteIcon(this.node, this.redNode, 'common/texture/red_point')
            }
